...
object TableSchemaFactory {
  ...
  fun <T> create(clazz: Class<*>): TableSchema<T> {
    return schemas.getOrPut(clazz) {
      ...
      // The modification is to use BeanTableSchemaParams in fromBean:
      //   Generally, this method should be preferred over fromBean(Class) because it allows
      ...
      if (clazz.getAnnotation<DynamoDbImmutable>(DynamoDbImmutable::class.java) != null) {
        TableSchema.fromImmutableClass(clazz)
